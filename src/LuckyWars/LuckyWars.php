						...
						if($config->get($arena . "inicio")==1){
							...
							if($timeToStart>0){
								...
								foreach($playersArena as $pl){
									$pl->sendPopup(TE::GREEN."Starting in".TE::GREEN . $timeToStart.TE::RESET);
									if($timeToStart<=5){
								...
								if($aop==1){
									...
									foreach($playersArena as $pl){
										$this->plugin->getServer()->broadcastMessage($this->prefix.$pl->getName().TE::GREEN." Won in the arena ".TE::AQUA.$arena);
										$pl->getInventory()->clearAll();
										$pl->removeAllEffects();
								...
								if($time == 779){
									...
									foreach($playersArena as $pl){
										...
										$pl->sendMessage(TE::YELLOW.">".TE::RED."Attention: ".TE::GREEN."The game has started");
								...
								if($time<300){
									...
									if($time <= 0){
										$this->plugin->getServer()->broadcastMessage($this->prefix .TE::GREEN."There are no winners in the arena ".TE::AQUA.$arena);
										foreach($playersArena as $pl){
											$pl->teleport($this->plugin->getServer()->getDefaultLevel()->getSafeSpawn(),0,0);
											$pl->getInventory()->clearAll();
											$pl->removeAllEffects();
							...
							foreach($playersArena as $pl){
								$pl->sendPopup(TE::DARK_AQUA . "Players Remaining" .TE::RESET);
							}
							$config->set($arena . "PlayTime", 780);
							$config->set($arena . "StartTime", 30);
						}
